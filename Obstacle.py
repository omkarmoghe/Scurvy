...
class Obstacle():
    def __init__(self, x_pos, file_name, damage_given, screen_size):
        ...
        num_objects = (screen_size[1] - 50) / object_rect.height  # number of objects
        num_objects -= 4
        # add object to the obstacle_objects list
        point = (x_pos, object_rect.width / 2)
        for i in range(0, num_objects):
            point = (random.randint(screen_size[0], screen_size[0] * 2.0), object_rect.height + 15 + point[1])
            obstacle = Object(point, file_name, velocity_ratio, (0, 0))
